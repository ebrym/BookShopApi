...
namespace BookShop.Api.IntegrationTest.AuthenticationHelper
{
    public class AuthHelper
    {
        ...
        CustomWebApplicationFactory<IntegrationTestStartup> factory, HttpClient client)
        {
            bool created = false;
        ...
        public async Task<CreateUserResponse> CreateUserForTest(CreateUserRequest newUser)
        {
            bool created = false;
            CreateUserRequest createUserRequest = null;
            ...
            try
            {
                ...
                var createdUserString = await createUserResponse.Content.ReadAsStringAsync();
                var creatdUserGenericResponse = JsonConvert.DeserializeObject<GenericResponse<CreateUserResponse>>(createdUserString);

                createdUser = (creatdUserGenericResponse.Data) as CreateUserResponse;
            }
            catch (Exception ex)
            {
            }
        ...
        public async Task<LoginToken> GetTokenForTestUser(CreateUserRequest createUserRequest)
        {
            LoginToken loginToken = new LoginToken();
            try
            {
            ...
            CustomWebApplicationFactory<IntegrationTestStartup> factory, HttpClient client)
        {
            ...
            try
            {
                CreateUserRequest createdUser = await this.CreateTestUser(createUserRequest, factory, client);
                loginToken = await this.GetLoginToken(createdUser);
            }
            catch (Exception ex)
            {
